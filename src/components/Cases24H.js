...
export default () => {
    ...
    // console.log("statedata", countryDataYD)
    ...
    const continentsum = d3Array.rollup(arrangeAllData(countriesByTodayCases), v => d3.sum(v, d => d.newcases), v => v.continent, v => v.country, v => v.globalpercent, v => v.countryflag);
    // const continentsum = d3Array.rollup(countryData, v => d3.sum(v, d => d.todayCases), v => v.continent, v => v.country, v => v.countryInfo);
    ...
    const frameProps = { 
        ...
        tooltipContent: d => (
            ...
            <h5 style={{margin:5}}>{Number(d.value).toLocaleString()} Total new cases</h5>
            <h5 style={{margin:5}}>{parseFloat(worldChange(d.value, hierarchydata.value))}% Global new cases</h5>
            </>
            : 
            <>
            <h3 style={{margin:5}}>{d.parent.parent[0]}</h3>
            <h5 style={{margin:5}}>{Number(d.value).toLocaleString()} Confirmed new cases</h5>
            <h5 style={{margin:5}}>{Number(d.parent[0]).toLocaleString()}% Global new cases</h5>
                    ...
                    <React.Fragment>
                        <div>
                            <Collapse in={expand}>
                                ...
                                >
                                    <AlertTitle>
                                        These are countries/regions with confirmed cases in the past 24 hours.
                                    </AlertTitle>
                                    <strong>
                                    ...
                                    <DialogContent dividers={scroll === 'paper'}>
                                        <TableContainer component={Paper}>
                                            <Table aria-label="simple table">
                                                ...
                                                <TableBody>
                                                    {arrangeAllData(countriesByTodayCases).map((data, index) => {
                                                        return (
                                                            <>
                                                                <TableRow key={index}>
                                                                    ...
                                                                    <TableCell align="right">{Number(data.newcases).toLocaleString()}</TableCell>
