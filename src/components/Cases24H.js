...
export default () => {
    ...
    const countriesByTodayCases = countryDataYD
        .filter(data => data.todayCases > 0)
        .sort((a, b) => b.todayCases - a.todayCases)
    // console.log("all confirmed todayCases", countriesByTodayCases);
    ...
    const arrangeAllData = (data) => {
        ...
        return data.map(datum => {
            return {
                globalpercent: parseFloat(worldChange(datum.todayCases, globalChange)),
    ...
    const arrangeRegionData = (data) => {
        let regionChange = data.reduce((a, c) => a + c.newcases, 0);
        return data.map(datum => {
            return {
                ...datum,
                regionpercent: contChange(datum.newcases, regionChange)
            }
        })
    };

    const covidContinents = d3Array.group(arrangeAllData(countriesByTodayCases), d => d.continent);
    // console.log("coviddata", covidContinents);

    const continentData = Array.from(covidContinents, ([key, value, globalvalue]) => ({
            key, 
            value: value.reduce((a, c) => a + c.newcases, 0), 
            globalvalue: value.reduce((a, c) => a + c.globalpercent, 0)
        }));
    // console.log("continentData", continentData)

    const continentChildren = Array.from(covidContinents, ([key, value, children]) => ({key, value: value.reduce((a, c) => a + c.newcases, 0), children: value}));
    ...
    // const continentsum = d3Array.rollup(arrangeAllData(countriesByTodayCases), v => d3.sum(v, d => d.newcases), v => v.continent, v => v.country);
    ...
    const LegendDemo = ({ title, children }) => {
        return (
          <div className="legend" style={{color:"white"}}>
            ...
            {children}
            <div>Updated: {arrangeAllData(countriesByTodayCases)[0].dateupdated}</div>
          </div>
        );
    }      
    ...
    const regionChange = (data) => {
        colorRegionChange = data.newcases === 0 ? "#FFFFFF" 
                                    ...
                                    <AlertTitle>
                                        {
                                            toggleRegion ? 
                                            <>
                                            These are the countries in {continentName} with new confirmed cases in the past 24 hours, compared globally
                                            </>
                                            :
                                            <>
                                            These are the countries in {continentName} with new confirmed cases in the past 24 hours, compared regionally
                                            </>
                                        }
                                    </AlertTitle>
                                    <strong>
                                            ...
                                            <Table aria-label="simple table">
                                                <TableHead>
                                                    <TableRow>
                                                        ...
                                                        <TableCell align="right">Total confirmed new cases</TableCell>
                                                        <TableCell align="right">% Global new cases</TableCell>
                                                        <TableCell align="right">% Of all new cases in {continentName}</TableCell>
                                                    </TableRow>
                                                </TableHead>
                                                    <TableBody>
                                        ...
                                        >
                                            ...
                                            <>
                                                ...
                                                <Tooltip title={
                                                    <Fragment>
                                                        <h2>{data.country}</h2>
                                                        <h2>Total confirmed new cases: {data.newcases}</h2>
                                                        <h2>% Of all new global cases: {data.globalpercent}%</h2>
                                        ...
                                        >
                                            ...
                                            <>
                                                ...
                                                <Tooltip title={
                                                    <Fragment>
                                                        <h2>{data.country}</h2>
                                                        <h2>Total new confirmed cases: {data.newcases}</h2>
                                                        <h2>% Of all new cases in {continentName}: {data.regionpercent}%</h2>
                                                        <img src={data.countryflag}/>
                                                    </Fragment>
                                                }>
                                                    <Circle
                    ...
                    <React.Fragment>
                        <div>
                            <Collapse in={expand}>
                                ...
                                >
                                    <AlertTitle>
                                        These are the top countries worldwide (max. 50) with the most confirmed cases in the past 24 hours 
                                    </AlertTitle>
                                    <strong>
                                        Mouse over data points for additional details.
                                    </strong>
                        ...
                        <div className="svg-menu">
                                        ...
                                        <TableContainer component={Paper}>
                                            <Table aria-label="simple table">
                                                <TableHead>
                                                    <TableRow>
                                                        <TableCell>Country</TableCell>
                                                        <TableCell align="right">Total confirmed new deaths</TableCell>
                                                        <TableCell align="right">% Global new deaths</TableCell>
                                                    </TableRow>
                                                </TableHead>
                                                    <TableBody>
                                                        {arrangeAllData(countriesByTodayCases.slice(0, 50)).map((data, index) => {
                                                            return (
                                                                <>
                                                                    <TableRow key={index}>
                                                                        <TableCell component="th" scope="row">
                            ...
                            <div className="transform-container">
                                <TransformComponent>
                                    <Svg width={window.innerWidth * .8} height={window.innerHeight * .7} className="svg-content">
                                        <Pack
                                            data={{children: arrangeAllData(countriesByTodayCases.slice(0, 50))}}
                                            sum={datum => datum.newcases}
                                            size={[window.innerWidth * .8, (window.innerHeight * .65)]}
                                            includeRoot={false}
                                            padding={5}
                                        >
                                            ...
                                            <>
                                                ...
                                                <Tooltip title={
                                                    <Fragment>
                                                        <h2>{data.country}</h2>
                                                        <h2>Total confirmed new cases: {data.newcases}</h2>
                                                        <h2>% Of all global new cases: {data.globalpercent.toFixed(4)}%</h2>
                                                        <img src={data.countryflag}/>
                                                    </Fragment>
                                                }>
                                                    <Circle
                    ...
                    <React.Fragment>
                        <div>
                            <Collapse in={expand}>
                                ...
                                >
                                    <AlertTitle>
                                        These are the continents with new confirmed cases in the past 24 hours
                                    </AlertTitle>
                                    <strong>
                                        Mouse over data points for additional details.
                                    </strong>
                                            ...
                                            <Table aria-label="simple table">
                                                <TableHead>
                                                    <TableRow>
                                                        ...
                                                        <TableCell align="right">Total confirmed new cases</TableCell>
                                                        <TableCell align="right">% Of all new global cases</TableCell>
                                                    </TableRow>
                                                </TableHead>
                                                    <TableBody>
                                        ...
                                        >
                                            ...
                                            <>
                                                ...
                                                <Tooltip title={
                                                    <Fragment>
                                                        <h2>{data.key}</h2>
                                                        <h2>Total confirmed new cases: {data.value}</h2>
                                                        <h2>% Of all new global cases: {data.globalvalue.toFixed(4)}%</h2>
                                                    </Fragment>
                                                }>
                                                    <Circle
                                                        key={key}
