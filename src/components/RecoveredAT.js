...
export default () => {
    ...
    const countriesByRecoveries = countryData
        .filter(data => data.recovered > 0)
        .sort((a, b) => b.recovered - a.recovered)
    // console.log("all confirmed recovered", countriesByRecoveries);
    ...
    const LegendDemo = ({ title, children }) => {
        return (
          <div className="legend" style={{color:"white"}}>
            ...
            {children}
            <div>Updated: {arrangeAllData(countriesByRecoveries)[0].dateupdated}</div>
          </div>
        );
    }      
    ...
    const frameProps = { 
        ...
        tooltipContent: d => (
            ...
            <h5 style={{margin:5}}>{Number(d.value).toLocaleString()} Total recoveries</h5>
            <h5 style={{margin:5}}>{parseFloat(worldChange(d.value, hierarchydata.value))}% Global recoveries</h5>
            </>
            : 
            <>
            <h3 style={{margin:5}}>{d.parent.parent[0]}</h3>
            <h5 style={{margin:5}}>{Number(d.value).toLocaleString()} Confirmed recoveries</h5>
            <h5 style={{margin:5}}>{Number(d.parent[0]).toLocaleString()}% Global recoveries</h5>
                    ...
                    <React.Fragment>
                        <div>
                            <Collapse in={expand}>
                                ...
                                >
                                    <AlertTitle>
                                        These are countries/regions with confirmed recoveries.
                                    </AlertTitle>
                                    <strong>
                                    ...
                                    <DialogContent dividers={scroll === 'paper'}>
                                        <TableContainer component={Paper}>
                                            <Table aria-label="simple table">
                                                ...
                                                <TableBody>
                                                    {arrangeAllData(countriesByRecoveries).map((data, index) => {
                                                        return (
                                                            <>
                                                                <TableRow key={index}>
                                                                    ...
                                                                    <TableCell align="right">{Number(data.recovered).toLocaleString()}</TableCell>
