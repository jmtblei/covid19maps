...
export default () => {
    ...
    const countriesByTodayDeaths = countryDataYD
        .filter(data => data.todayDeaths > 0)
        .sort((a, b) => b.todayDeaths - a.todayDeaths)
    // console.log("all confirmed todayDeaths", countriesByTodayDeaths);
    ...
    const arrangeAllData = (data) => {
        ...
        return data.map(datum => {
            return {
                globalpercent: parseFloat(worldChange(datum.todayDeaths, globalChange)),
    ...
    const arrangeRegionData = (data) => {
        let regionChange = data.reduce((a, c) => a + c.newdeaths, 0);
        return data.map(datum => {
            return {
                ...datum,
                regionpercent: contChange(datum.newdeaths, regionChange)
            }
        })
    };

    const covidContinents = d3Array.group(arrangeAllData(countriesByTodayDeaths), d => d.continent);
    // console.log("coviddata", covidContinents);

    const continentData = Array.from(covidContinents, ([key, value, globalvalue]) => ({
            key, 
            value: value.reduce((a, c) => a + c.newdeaths, 0), 
            globalvalue: value.reduce((a, c) => a + c.globalpercent, 0)
        }));
    // console.log("continentData", continentData)

    const continentChildren = Array.from(covidContinents, ([key, value, children]) => ({key, value: value.reduce((a, c) => a + c.newdeaths, 0), children: value}));
    ...
    // const continentsum = d3Array.rollup(arrangeAllData(countriesByTodayDeaths), v => d3.sum(v, d => d.newdeaths), v => v.continent, v => v.country);
    ...
    const LegendDemo = ({ title, children }) => {
        return (
          <div className="legend" style={{color:"white"}}>
            ...
            {children}
            <div>Updated: {arrangeAllData(countriesByTodayDeaths)[0].dateupdated}</div>
          </div>
        );
    }      
    ...
    const globalChange = (data) => {
        switch (selection) {
            case "Continents": {
                colorChange = data.newdeaths === 0 ? "#FFFFFF" 
    ...
    const regionChange = (data) => {
        colorRegionChange = data.newdeaths === 0 ? "#FFFFFF" 
                                    ...
                                    <AlertTitle>
                                        {
                                            toggleRegion ? 
                                            <>
                                            These are the countries in {continentName} with new confirmed deaths in the past 24 hours, compared globally
                                            </>
                                            :
                                            <>
                                            These are the countries in {continentName} with new confirmed deaths in the past 24 hours, compared regionally
                                            </>
                                        }
                                    </AlertTitle>
                                    <strong>
                                            ...
                                            <Table aria-label="simple table">
                                                <TableHead>
                                                    <TableRow>
                                                        ...
                                                        <TableCell align="right">% Of all new deaths in {continentName}</TableCell>
                                                    </TableRow>
                                                </TableHead>
                                                    <TableBody>
                                        ...
                                        >
                                            ...
                                            <>
                                                ...
                                                <Tooltip title={
                                                    <Fragment>
                                                        <h2>{data.country}</h2>
                                                        <h2>Total confirmed new deaths: {data.newdeaths}</h2>
                                                        <h2>% Of all new global deaths: {data.globalpercent}%</h2>
                                            ...
                                            <>
                                                ...
                                                <Tooltip title={
                                                    <Fragment>
                                                        <h2>{data.country}</h2>
                                                        <h2>Total confirmed new deaths: {data.newdeaths}</h2>
                                                        <h2>% Of all new deaths in {continentName}: {data.regionpercent}%</h2>
                                                        <img src={data.countryflag}/>
                                                    </Fragment>
                                                }>
                                                    <Circle
                    ...
                    <React.Fragment>
                        <div>
                            <Collapse in={expand}>
                                ...
                                >
                                    <AlertTitle>
                                        These are the top countries worldwide (max. 50) with the most confirmed deaths in the past 24 hours 
                                    </AlertTitle>
                                    <strong>
                                        Mouse over data points for additional details.
                                    </strong>
                        ...
                        <div className="svg-menu">
                                        ...
                                        <TableContainer component={Paper}>
                                            <Table aria-label="simple table">
                                                <TableHead>
                                                    ...
                                                    <TableBody>
                                                        {arrangeAllData(countriesByTodayDeaths.slice(0, 50)).map((data, index) => {
                                                            return (
                                                                <>
                                                                    <TableRow key={index}>
                                                                        <TableCell component="th" scope="row">
                            ...
                            <div className="transform-container">
                                <TransformComponent>
                                    <Svg width={window.innerWidth * .8} height={window.innerHeight * .7} className="svg-content">
                                        <Pack
                                            data={{children: arrangeAllData(countriesByTodayDeaths.slice(0, 50))}}
                                            sum={datum => datum.newdeaths}
                                            size={[window.innerWidth * .8, (window.innerHeight * .65)]}
                                            includeRoot={false}
                                            padding={5}
                                        >
                                            ...
                                            <>
                                                ...
                                                <Tooltip title={
                                                    <Fragment>
                                                        <h2>{data.country}</h2>
                                                        <h2>Total confirmed new deaths: {data.newdeaths}</h2>
                                                        <h2>% Of all global new deaths: {data.globalpercent.toFixed(4)}%</h2>
                                                        <img src={data.countryflag}/>
                                                    </Fragment>
                                                }>
                                                    <Circle
                    ...
                    <React.Fragment>
                        <div>
                            <Collapse in={expand}>
                                ...
                                >
                                    <AlertTitle>
                                        These are the continents with new confirmed deaths in the past 24 hours
                                    </AlertTitle>
                                    <strong>
                                        Mouse over data points for additional details.
                                    </strong>
                                            ...
                                            <Table aria-label="simple table">
                                                <TableHead>
                                                    <TableRow>
                                                        <TableCell>Continent</TableCell>
                                                        <TableCell align="right">Total confirmed new deaths</TableCell>
                                                        <TableCell align="right">% Of all new global deaths</TableCell>
                                                    </TableRow>
                                                </TableHead>
                                                    <TableBody>
                                        ...
                                        >
                                            ...
                                            <>
                                                ...
                                                <Tooltip title={
                                                    <Fragment>
                                                        <h2>{data.key}</h2>
                                                        <h2>Total confirmed new deaths: {data.value}</h2>
                                                        <h2>% Of all new global deaths: {data.globalvalue.toFixed(4)}%</h2>
                                                    </Fragment>
                                                }>
                                                    <Circle
                                                        key={key}
